"""
BRONE DITER BATTERY MASTER - TAHAP 3 (PERSISTENCE)
- Methodology: DITER (Voltage Sag + Kt Logic + UVLO Protection)
- Fitur Baru: Pilih Mode Baterai saat Startup (L1: Resume, R1: Full Charge)
- Auto-Save: Menyimpan kondisi baterai setiap detik ke file .txt
"""
...
class BroneDiterBattery:
    def __init__(self):
        ...
        # PARAMETER KEAMANAN (UVLO)
        # Batas aman LiPo: 3.3V per sel. Total 6 sel = 19.8V
        self.CUTOFF_VOLTAGE = 19.8 
        
        # DITER PARAMETER: Internal Resistance & Voltage Curve
        # Estimasi R_int total (Baterai + Kabel + BMS) ~ 0.15 Ohm
        self.R_INTERNAL = 0.15 
        
        # Konversi ke Energi Total (Joule)
        # Asumsi rata-rata tegangan discharge efektif ~21.0V untuk perhitungan kapasitas Wh konservatif
        self.total_energy_capacity = (21.0 * (self.BATT_CAPACITY_MAH / 1000.0)) * 3600.0
        
        # Init energy sementara (akan di-overwrite oleh user selection saat run)
        self.current_energy = self.total_energy_capacity 

        # --- B. SPESIFIKASI BEBAN ELEKTRONIK (Static Load) ---
        # Constant Power Load (CPL): Regulator akan menarik arus lebih besar saat voltase turun
        self.P_STATIC = 8.0 # Watt (Orange Pi 5 + ESP32)

        # --- C. SPESIFIKASI MOTOR (PG36 24V - DITER Physics Model) ---
        self.I_IDLE = 0.25        # Ampere
        self.I_STALL = 2.5        # Ampere
        self.TORQUE_STALL = 1.96  # N.m
        self.MAX_SPEED = 46.0     # Rad/s
        
        # Hitung Torque Constant (Kt)
        # Kt = Torque / (Current - No_Load_Current)
        ...
        if pygame.joystick.get_count() > 0:
            ...
            print(f">> DITER MODEL: Active (Voltage Sag + Kt Logic + UVLO {self.CUTOFF_VOLTAGE}V)")
        else:
    ...
    def get_input(self):
        ...
        # Deteksi tombol untuk rotasi (tidak digunakan saat menu selection)
    ...
    def save_battery_state(self):
        """Menyimpan sisa energi (Joule) ke file txt"""
        try:
    ...
    def load_battery_state(self):
        """Membaca sisa energi dari file txt"""
    ...
    def wait_for_user_selection(self):
        """
        Looping awal: Robot diam sampai user memilih mode baterai.
        L1 (Button 6) = Resume (Belum dicas)
        R1 (Button 7) = Reset (Sudah dicas penuh)
        """
        print("\n=========================================")
        ...
        while self.robot.step(self.timestep) != -1:
            ...
            if not self.js: 
                ...
                return # Default full jika tidak ada joystick

            # Cek Button L1 (Biasanya index 4 atau 6 tergantung gamepad)
            ...
            # Cek Button R1 (Biasanya index 5 atau 7 tergantung gamepad)
        ...
        if selected:
            # Beri jeda sedikit agar tombol tidak terdeteksi sebagai gerakan rotasi awal
    ...
    def run(self):
        # 1. TUNGGU PILIHAN USER (L1/R1) SEBELUM MULAI
        ...
        while self.robot.step(self.timestep) != -1:
            ...
            if voltage <= self.CUTOFF_VOLTAGE:
                ...
                self.robot_stop()
                self.save_battery_state() # Save kondisi terakhir (mati)
                break
            ...
            if t - self.last_log > 1.0:
                ...
                # Menyimpan state agar jika crash/close, data aman
                self.save_battery_state()
